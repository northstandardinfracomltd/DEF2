...
import { CompanyInfo, Member, MemberSchedule, MemberAbsence, WebsiteSettings, WebsiteSectionItem } from '../types';
...
import { REGIONS_FRANCAISES, getLocationCustomName, slugify } from '../utils';
...
export default function SettingsModal({
  ...
  // Website Exploitants settings states
  const [copiedUrl, setCopiedUrl] = React.useState(false);
  const [expandedWebsiteSec, setExpandedWebsiteSec] = React.useState<string | null>(null);

  const handleCopyWebsiteUrl = () => {
    const slug = slugify(localCompany.name || '');
    const url = `https://defibeo.deroesch.com/${slug}`;
    navigator.clipboard.writeText(url);
    setCopiedUrl(true);
    setTimeout(() => setCopiedUrl(false), 2000);
  };

  const updateWebsiteSetting = (key: keyof WebsiteSettings | 'enablePublicWebsite', value: any) => {
    if (key === 'enablePublicWebsite') {
      setLocalCompany(prev => ({
        ...prev,
        enablePublicWebsite: value
      }));
    } else {
      setLocalCompany(prev => ({
        ...prev,
  ...
  const updateWebsiteSectionItem = (section: 'sec2' | 'sec4', index: number, field: keyof WebsiteSectionItem, value: string) => {
    setLocalCompany(prev => {
      const settings = prev.websiteSettings || {};
      const key = section === 'sec2' ? 'sec2Items' : 'sec4Items';
  ...
  const addWebsiteSectionItem = (section: 'sec2' | 'sec4') => {
    setLocalCompany(prev => {
      const settings = prev.websiteSettings || {};
      const key = section === 'sec2' ? 'sec2Items' : 'sec4Items';
      const items = [...(settings[key] || [])];
      ...
      items.push({
        ...
        title: "",
        description: "",
        imageUrl: section === 'sec2' ? "" : undefined
      });
      return {
        ...prev,
        websiteSettings: {
  ...
  const removeWebsiteSectionItem = (section: 'sec2' | 'sec4', index: number) => {
    setLocalCompany(prev => {
      const settings = prev.websiteSettings || {};
      const key = section === 'sec2' ? 'sec2Items' : 'sec4Items';
      const items = [...(settings[key] || [])];
    ...
    <div className={isPage ? "w-full min-h-screen bg-white font-sans text-black" : "fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 sm:p-6 shadow-2xl"} id={isPage ? "settings-page-wrapper" : "settings-modal-overlay"}>
      ...
      >
        ...
        >
          ...
          {/* SECTION 7: SITE WEB EXPLOITANT */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 text-left mt-4 animate-fadeIn" id="settings-section-website" style={{ order: 98 }}>
            {renderSectionHeader(t("Site web exploitant"), false)}
            
            <p className="text-[16px] text-slate-500 font-sans leading-relaxed">
              {t("Activez et gérez un mini-site internet commercial pour votre entreprise. Les visiteurs pourront consulter vos produits, vos offres, votre FAQ et vous contacter directement.")}
            </p>

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-100">
              <span className="font-bold text-[16px] text-black font-sans">
                {t("Activer le site internet public")}
              </span>
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => updateWebsiteSetting('enablePublicWebsite', 'Non')}
                  className={`px-4 py-1.5 text-xs font-semibold rounded-lg border cursor-pointer font-sans transition-all ${
                    (localCompany.enablePublicWebsite || 'Non') === 'Non'
                ...
                <button
                  type="button"
                  onClick={() => updateWebsiteSetting('enablePublicWebsite', 'Oui')}
                  className={`px-4 py-1.5 text-xs font-semibold rounded-lg border cursor-pointer font-sans transition-all ${
                    (localCompany.enablePublicWebsite || 'Non') === 'Oui'
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1">
                <span className="text-[14px] font-bold text-slate-700 font-sans">
                  {t("URL publique de votre site")}
                </span>
                <div className="font-mono text-xs text-slate-600 break-all">
                  defibeo.deroesch.com/{slugify(localCompany.name || '')}
                </div>
              </div>
              <button
                type="button"
                onClick={handleCopyWebsiteUrl}
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-lg transition-all cursor-pointer inline-flex items-center gap-1.5 self-start sm:self-center"
              >
                {copiedUrl ? t("Copié !") : t("Copier l’URL")}
              </button>
            </div>

            {/* Section Collapsible Editors */}
            <div className="space-y-3 pt-2">
              <h4 className="font-bold text-[16px] text-black font-sans">
                {t("Personnaliser le contenu des sections")} :
              </h4>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec1' ? null : 'sec1')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 1 : Présentation")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec1' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec1' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-slate-700">{t("Activer cette section")}</span>
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec1Active !== false} 
                        onChange={e => updateWebsiteSetting('sec1Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Titre")}</label>
                      <input 
                        type="text" 
                        placeholder={t("Bienvenue chez...")}
                        value={localCompany.websiteSettings?.sec1Title || ''} 
                        onChange={e => updateWebsiteSetting('sec1Title', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Sous-titre")}</label>
                      <input 
                        type="text" 
                        placeholder={t("Votre partenaire de confiance...")}
                        value={localCompany.websiteSettings?.sec1Subtitle || ''} 
                        onChange={e => updateWebsiteSetting('sec1Subtitle', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Description")}</label>
                      <textarea 
                        rows={3}
                        placeholder={t("Présentez votre entreprise et vos services...")}
                        value={localCompany.websiteSettings?.sec1Desc || ''} 
                        onChange={e => updateWebsiteSetting('sec1Desc', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("URL de l'image")}</label>
                      <input 
                        type="text" 
                        placeholder="https://images.unsplash.com/..."
                        value={localCompany.websiteSettings?.sec1Image || ''} 
                        onChange={e => updateWebsiteSetting('sec1Image', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                  </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec2' ? null : 'sec2')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 2 : Défibrillateurs Connectés")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec2' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec2' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-4 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec2Active !== false} 
                        onChange={e => updateWebsiteSetting('sec2Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Titre de la section")}</label>
                      <input 
                        type="text" 
                        placeholder={t("Nos Défibrillateurs Connectés")}
                        value={localCompany.websiteSettings?.sec2Title || ''} 
                        onChange={e => updateWebsiteSetting('sec2Title', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Sous-titre de la section")}</label>
                      <input 
                        type="text" 
                        placeholder={t("Découvrez notre matériel...")}
                        value={localCompany.websiteSettings?.sec2Subtitle || ''} 
                        onChange={e => updateWebsiteSetting('sec2Subtitle', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                        <span className="font-bold text-xs uppercase text-slate-500">{t("Éléments de la grille")} ({(localCompany.websiteSettings?.sec2Items || []).length}/10)</span>
                        <button
                          type="button"
                          onClick={() => addWebsiteSectionItem('sec2')}
                          disabled={(localCompany.websiteSettings?.sec2Items || []).length >= 10}
                          className="px-2.5 py-1 bg-slate-900 hover:bg-slate-800 disabled:opacity-40 text-white rounded text-xs font-semibold cursor-pointer"
                        >
                          + {t("Ajouter")}
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                        {(localCompany.websiteSettings?.sec2Items || []).map((item, idx) => (
                          <div key={item.id} className="p-3 bg-slate-50 border border-slate-100 rounded-lg space-y-2 relative">
                            <button
                              type="button"
                              onClick={() => removeWebsiteSectionItem('sec2', idx)}
                              className="absolute top-2 right-2 text-red-500 hover:text-red-700 text-xs font-bold cursor-pointer"
                            >
                              {t("Supprimer")}
                            </button>
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">{t("Titre de l'élément")}</label>
                              <input 
                                type="text" 
                                placeholder={t("Ex: Défibrillateur DAE Premium")}
                                value={item.title} 
                                onChange={e => updateWebsiteSectionItem('sec2', idx, 'title', e.target.value)}
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs bg-white text-black"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">{t("Description")}</label>
                              <input 
                                type="text" 
                                placeholder={t("Ex: Un appareil intelligent de dernière génération...")}
                                value={item.description} 
                                onChange={e => updateWebsiteSectionItem('sec2', idx, 'description', e.target.value)}
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs bg-white text-black"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">{t("URL de l'image")}</label>
                              <input 
                                type="text" 
                                placeholder="https://images.unsplash.com/..."
                                value={item.imageUrl || ''} 
                                onChange={e => updateWebsiteSectionItem('sec2', idx, 'imageUrl', e.target.value)}
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs bg-white text-black"
                              />
                            </div>
                          </div>
                        ))}
                        {(localCompany.websiteSettings?.sec2Items || []).length === 0 && (
                          <p className="text-xs text-slate-400 italic text-center py-2">{t("Aucun élément configuré (des valeurs d'exemple seront affichées par défaut).")}</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec3' ? null : 'sec3')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 3 : Offres et services")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec3' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec3' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec3Active !== false} 
                        onChange={e => updateWebsiteSetting('sec3Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Contrat de Maintenance...")}
                        value={localCompany.websiteSettings?.sec3Title || ''} 
                        onChange={e => updateWebsiteSetting('sec3Title', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Tranquillité d'esprit...")}
                        value={localCompany.websiteSettings?.sec3Subtitle || ''} 
                        onChange={e => updateWebsiteSetting('sec3Subtitle', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <textarea 
                        rows={3}
                        placeholder={t("Détaillez vos services, contrats...")}
                        value={localCompany.websiteSettings?.sec3Desc || ''} 
                        onChange={e => updateWebsiteSetting('sec3Desc', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder="https://images.unsplash.com/..."
                        value={localCompany.websiteSettings?.sec3Image || ''} 
                        onChange={e => updateWebsiteSetting('sec3Image', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                  </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec4' ? null : 'sec4')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 4 : FAQ et ressources")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec4' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec4' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-4 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec4Active !== false} 
                        onChange={e => updateWebsiteSetting('sec4Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Questions Fréquentes")}
                        value={localCompany.websiteSettings?.sec4Title || ''} 
                        onChange={e => updateWebsiteSetting('sec4Title', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Retrouvez les réponses à vos questions...")}
                        value={localCompany.websiteSettings?.sec4Subtitle || ''} 
                        onChange={e => updateWebsiteSetting('sec4Subtitle', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                        <span className="font-bold text-xs uppercase text-slate-500">{t("Questions de la FAQ")} ({(localCompany.websiteSettings?.sec4Items || []).length}/10)</span>
                        <button
                          type="button"
                          onClick={() => addWebsiteSectionItem('sec4')}
                          disabled={(localCompany.websiteSettings?.sec4Items || []).length >= 10}
                          className="px-2.5 py-1 bg-slate-900 hover:bg-slate-800 disabled:opacity-40 text-white rounded text-xs font-semibold cursor-pointer"
                        >
                          + {t("Ajouter")}
                        </button>
                      ...
                      <div className="space-y-4">
                        {(localCompany.websiteSettings?.sec4Items || []).map((item, idx) => (
                          <div key={item.id} className="p-3 bg-slate-50 border border-slate-100 rounded-lg space-y-2 relative">
                            <button
                              type="button"
                              onClick={() => removeWebsiteSectionItem('sec4', idx)}
                              className="absolute top-2 right-2 text-red-500 hover:text-red-700 text-xs font-bold cursor-pointer"
                            >
                              {t("Supprimer")}
                            </button>
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">{t("Question (Petit-titre)")}</label>
                              <input 
                                type="text" 
                                placeholder={t("Ex: Quelle est la réglementation ERP ?")}
                                value={item.title} 
                                onChange={e => updateWebsiteSectionItem('sec4', idx, 'title', e.target.value)}
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs bg-white text-black"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">{t("Réponse (Description)")}</label>
                              <textarea 
                                rows={2}
                                placeholder={t("Ex: Depuis le décret de 2018...")}
                                value={item.description} 
                                onChange={e => updateWebsiteSectionItem('sec4', idx, 'description', e.target.value)}
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs bg-white text-black"
                              />
                            </div>
                          </div>
                        ))}
                        {(localCompany.websiteSettings?.sec4Items || []).length === 0 && (
                          <p className="text-xs text-slate-400 italic text-center py-2">{t("Aucune question configurée (des valeurs d'exemple seront affichées par défaut).")}</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec5' ? null : 'sec5')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 5 : Actualités et réseaux")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec5' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec5' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec5Active !== false} 
                        onChange={e => updateWebsiteSetting('sec5Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Actualités et Réseaux Sociaux")}
                        value={localCompany.websiteSettings?.sec5Title || ''} 
                        onChange={e => updateWebsiteSetting('sec5Title', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        type="text" 
                        placeholder={t("Restez informé de nos actualités...")}
                        value={localCompany.websiteSettings?.sec5Subtitle || ''} 
                        onChange={e => updateWebsiteSetting('sec5Subtitle', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <textarea 
                        rows={3}
                        placeholder={t("Suivez nos interventions sur le terrain...")}
                        value={localCompany.websiteSettings?.sec5Desc || ''} 
                        onChange={e => updateWebsiteSetting('sec5Desc', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2 border-t border-slate-100 pt-3">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Lien Facebook")}</label>
                      <input 
                        type="text" 
                        placeholder="facebook.com/votrepage"
                        value={localCompany.websiteSettings?.sec5Fb || ''} 
                        onChange={e => updateWebsiteSetting('sec5Fb', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Lien Instagram")}</label>
                      <input 
                        type="text" 
                        placeholder="instagram.com/votrepage"
                        value={localCompany.websiteSettings?.sec5Insta || ''} 
                        onChange={e => updateWebsiteSetting('sec5Insta', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-bold text-slate-700 uppercase">{t("Lien LinkedIn")}</label>
                      <input 
                        type="text" 
                        placeholder="linkedin.com/company/votrepage"
                        value={localCompany.websiteSettings?.sec5In || ''} 
                        onChange={e => updateWebsiteSetting('sec5In', e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs"
                      />
                    </div>
                  </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <button
                  type="button"
                  onClick={() => setExpandedWebsiteSec(prev => prev === 'sec6' ? null : 'sec6')}
                  className="w-full px-4 py-3 flex items-center justify-between text-left bg-slate-50 hover:bg-slate-100 font-semibold text-slate-800 text-sm transition-all cursor-pointer"
                >
                  <span>{t("Section 6 : Contact")}</span>
                  <span className="text-xs text-slate-500">
                    {expandedWebsiteSec === 'sec6' ? '▲' : '▼'}
                  </span>
                </button>
                {expandedWebsiteSec === 'sec6' && (
                  <div className="p-4 bg-white border-t border-slate-100 space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <input 
                        type="checkbox" 
                        checked={localCompany.websiteSettings?.sec6Active !== false} 
                        onChange={e => updateWebsiteSetting('sec6Active', e.target.checked)}
                        className="h-4 w-4 text-blue-600 rounded cursor-pointer"
                      />
                    </div>
                    <p className="text-xs text-slate-500 italic leading-relaxed">
                      {t("Cette section affiche vos coordonnées publiques (Email et Téléphone de l'entreprise) configurés dans les Réglages ci-dessus, et propose un formulaire de contact permettant à vos clients ou prospects de vous laisser un message. Les messages reçus arriveront instantanément dans l'onglet CRM (Leads/Tickets) de votre logiciel Défibeo.")}
                    </p>
                  </div>
                )}
              </div>
